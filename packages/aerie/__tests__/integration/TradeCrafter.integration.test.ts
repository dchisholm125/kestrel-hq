...
describe('TradeCrafter (integration)', () => {
  ...
  test('crafts backrun transaction for a created swap', async () => {
    ...
    const crafted = await crafter.craftBackrunFromCandidate(opp!, await signer.getAddress());
    console.log('[integration][TradeCrafter] craftBackrunFromCandidate result', crafted ? 'tx object' : 'null');
